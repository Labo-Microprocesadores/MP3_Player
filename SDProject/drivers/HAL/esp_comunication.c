...
static void parse_msg(void)
{
	...
	if(len == 1)
	{
		...
		if(buffer <= 40){
			...
			case 44: emitEvent(PREV_SONG_EV) /*Audio_playPrevFile()*/; break;//prev
			case 45: emitEvent(NEXT_SONG_EV) /*Audio_playNextFile()*/; break;//next
...
/*
bool msg_IsAnyMsg(void)
{
	return UART_is_rx_msg(3);
}

void msg_sendMsg(const char* msg, uint8_t cant)
{

}

uint8_t msg_getMsg(char* buffer, uint8_t max_cant)
{
	return UART_read_msg(3, buffer, max_cant);
}*/
