...
{
    FRESULT fr = f_open(decoder.mp3File, filename, FA_READ);
    if (fr == FR_OK)
        return true;
    return false;
}
...
size_t readFile(void* buf, size_t count)
{
    size_t read;
    FRESULT fr;
    fr = f_read(decoder.mp3File, ((uint8_t*)buf), count, &read);
    if (fr == FR_OK)
        return read;
    return 0;
}
...
void readID3Tag(void)
{
    ...
    if (has_ID3_tag(decoder.mp3File))
    {
        ...
        f_lseek(decoder.mp3File, tagSize);
